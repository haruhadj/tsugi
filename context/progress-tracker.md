...
| 7 — List import | **In progress** — started 2026-08-15. `tokens.ts`, `anilist.ts`, `mal.ts`, `GET /api/lists/:provider/:mediaType` all implemented; `tsc`/`eslint` clean; invariant 10 and criterion 11 verified via grep. Production OAuth verified: both providers' redirect URIs follow `/api/auth/oauth2/callback/{providerId}`, confirmed live against `https://tsugi-lyart.vercel.app`; manual dashboard registration of those URIs still pending. All four service/route files now have unit-tier test coverage: `mal.ts` (`mal.test.ts`, 9 tests), `lists.ts` (`lists.db.test.ts`, 3 tests, scoped to the no-session 401, invariant 10, and `?refresh=1` not bypassing the session check), `anilist.ts` (`anilist.test.ts`, 6 tests), and `tokens.ts` (`tokens.test.ts`, 7 tests, using `bun:test`'s `mock.module` to fake `@/db` — a first in this codebase). Full-path success coverage for all of these still needs a real linked account, folded into criteria 1–8. **My-list picker UI implemented:** `MyListPicker.tsx` (new) + `RecBuilder.tsx` mode toggle, `handleImport`, and `authClient.listAccounts()`-based criterion-9 gating, plus (2026-08-16 evening) a refresh button and stale-cache notice. **Update, 2026-08-16 evening: production 500s fixed, caching redesigned.** Both AniList and MAL imports were 500ing in production because migration `0002_broken_triton.sql` (creates `list_cache`) had never been applied to the production Supabase DB — fixed by running `bun x drizzle-kit migrate` against production. The original 5-minute-TTL cache design was replaced with cache-once-then-explicit-refresh: `lists.ts` serves the cached row unconditionally unless `?refresh=1` is passed, and falls back to the existing stale cache instead of erroring if a forced refresh's provider call fails. `tsc`/`eslint` clean; full suite (`bun test --conditions=react-server`): 149 pass, 0 fail. Committed as `5ae4181`, pushed to `main`. Remaining: account-gated criteria 1–8 (blocked on manual OAuth registration), criterion 12 live timed pass |
| 8 — Dashboard | **Implementation complete** — 2026-08-16. `/dashboard`, `DashboardRecList.tsx`, `GET`/`DELETE /api/recs`, and `ProviderConnections.tsx` unlink UI (with last-provider guard, criterion 10) all built. `recs.db.test.ts` adds 6 new in-process tests (session-scoped listing/newest-first/no-leakage, 401 boundaries, cross-user 403-not-404, delete-then-404) covering criteria 1, 3, 6, 7 — 10/10 passing after fixing a test bug (generated slugs exceeded the `varchar(12)` column limit on `recommendation.slug`). Full suite: 153/153 pass, 0 fail; `tsc`/`eslint` clean (criterion 14). Criteria 1/6's "two real accounts" wording was satisfied via mocked sessions, not two literal browser logins — flagged in [PHASE-8.md](./planning/PHASE-8.md) rather than claimed as identical evidence. Criteria 2, 4, 12, 13 not re-verified live in a browser this session. |
