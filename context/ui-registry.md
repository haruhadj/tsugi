...
**Updated 2026-08-23 for D59 (list editing)** — no new component. `ListBuilder` gained an
optional `existing` prop and a second call site (`/r/[slug]/edit`); `DashboardRecList` and
`RecView` gained links into it.
...
| `Header` | `src/components/Header.tsx` | `username: string \| null` | every page | Client component. Sticky blurred top bar, its `NAV` glyphs now rendered on desktop as well as in the mobile bar (tinted `text-primary` when active, alongside — never instead of — the `.brand-gradient` underline) plus a fixed mobile tab bar under `md`. `username` is null for signed-out visitors and renders a Sign in button — the product is readable without an account (invariant 9). The bottom bar is fixed, so the page clearance lives as `pb-rail` on `<body>` in `layout.tsx`, not as a spacer inside this component. `--rail` is the bar's `min-h-14` plus `env(safe-area-inset-bottom)`, collapsing to `0px` at `md` in `globals.css` so no call site needs an override. **The top bar's `h-16` is load-bearing beyond this component** — `FeedList`'s sticky sort rail pins at `top-16` to sit directly under it, and the two are coupled by a literal, not a token |
...
| `ListBuilder` | `src/components/ListBuilder.tsx` | `existing?: BuilderList` | `src/app/page.tsx`, `/r/[slug]/edit` | Client component. Prototype's two-column workspace at `lg`: toolbar (Social card / Save draft / Publish), left metadata panel (title, category `Select`, genre cloud, caption, note), right search + tray. Composes `MediaSearchInput` + `MyListPicker` + `ItemTray` + `SegmentedRadioGroup` + `SocialCardPreview`. Takes **no** `scoreFormat` prop since D47 — typed scores are always `POINT_10`. Pre-checks invariant 8 client-side; surfaces recoverable failures through `Alert`, saved-draft through a `role="status"` region. POSTs `/api/lists` once with `publish` set, then opens `ShareModal`. **`existing` puts it in edit mode (D59)** — its presence *is* the flag, there is no `mode` prop. State is seeded from it once, with no syncing effect (the prop comes from a server component that re-renders only on navigation, and an effect would fight the user's typing). Edit mode swaps the step footer for Cancel / Save changes, shown on **every** step rather than only the last, and PATCHes `/api/lists/:slug` with the whole list before navigating to `/r/[slug]`. Publishing is not part of it. Also exports `toTrayItems(ListView["items"])`, which the edit page uses to seed the tray |
| `RecView` | `src/components/RecView.tsx` | `rec: ListView` | `/r/[slug]` | **Server component, and must stay one** — it calls `getEnv()`, which validates the whole server env, so a `"use client"` here throws the moment `process.env` carries only the `NEXT_PUBLIC_*` keys. The artifact card. Builds the share URL from `NEXT_PUBLIC_APP_URL` (never `window.location`) plus the markdown and card payloads, and hands them to client leaves. No `viewerId` and no comment loading since **D46**. Renders an **Edit** link beside the quick actions when `rec.isOwner` (D59) — an affordance only; the access check is `/r/[slug]/edit`'s own |
...
| `VotePill` | `src/components/VotePill.tsx` | `score`, `direction`, `onVote`, `disabled?`, `orientation?`, `size?`, `className?` | `VoteButtons` | Client component, presentational and fully controlled — it owns no score and makes no request, so a second caller can post to a different endpoint. Kept split from `VoteButtons` for that reason even though comments (**D46**) removed the second caller. Active state is colour **and** `aria-pressed` **and** a thicker icon stroke. `size` is `sm | md | touch`; `touch` (**D58**) is `md` with 44px room around the arrows on a phone, collapsing back to exactly `md` from the `md` breakpoint up — the arrows are the most-tapped controls in the product and a mis-hit is a write |
...
| `VoteButtons` | `src/components/VoteButtons.tsx` | `slug`, `initialScore`, `orientation?`, `size?`, `className?` | `FeedList` | `src/components/FeedList.tsx` | `entries: FeedEntry[]`, `sortNav: ReactNode`, `filterBar?: ReactNode`, `browseDrawer?: ReactNode`, `pagination?: ReactNode`, `urlState: FeedUrlState` | `/feed` | Client component. The stream / compact / grid densities; the toggle is `md`-and-up only since **D58**, because the stream card is already the compact reading on a phone. Density is client state and deliberately **not** a URL param. Compact and grid are fully clickable via the **link-overlay** pattern (ui-rules.md); stream is overlaid **below `md` only** (`md:after:content-none`) — the compact card sheds the affordances that made a card-wide target unsafe on the desktop one. Owns two hooks: `useInfiniteFeed` appends later pages from `GET /api/feed` via an `IntersectionObserver` sentinel, and `usePullToRefresh` re-renders the server component on a downward drag at scroll top. **`FeedPage` must give this a `key` derived from the query** — appended pages are only meaningful for the filter they were fetched under, and a remount discards them where an effect would clear them a render late. `pagination` is dropped once a page has been appended, and is the no-JS route. No slot numbers since D51 |
...
| `DashboardRecList` | `src/components/DashboardRecList.tsx` | `initialRecs: ListView[]` | `/dashboard` | Client component. All/Published/Drafts filter over already-fetched rows, plus per-row publish/unpublish, duplicate, open, an edit link, and a two-press delete. Duplicate calls `router.refresh()` rather than guessing the server-assigned slug. Edit is a `Link` inside an `asChild` `Button`, not a handler — it navigates, so middle-click and open-in-new-tab work and `busySlug` does not disable it (D59) |
