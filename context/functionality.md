...
| ~~**Comments, replies, likes on a rec page**~~ | ~~Turns a share target into a social network, bringing moderation, spam, and abuse reporting — permanent cost against a promise about speed.~~ **Fully reversed (D43, then D44):** D43 added voting; D44 added free-text comments with one level of replies and per-comment voting. The moderation cost the original entry named is now real and **unpaid** — there is no report, block, or admin surface, and a comment can only be removed by its author. See D44's "Revisit if". |
